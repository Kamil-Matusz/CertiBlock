...
public class MetricPublisher : IDisposable
{
    ...
    public void Publish(MetricFinalizedEvent metric)
    {
        ...
        try
        {
            ...
            _channel.BasicPublish(
                exchange: "",
                routingKey: "certiblock.finalization.polygon",
                basicProperties: props,
                body: body
            );

            _channel.WaitForConfirmsOrDie(TimeSpan.FromSeconds(5));
            _logger.LogInformation("Published Polygon finalization metric for certificate {CertificateId}",
                                    metric.CertificateId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to publish Polygon finalization metric for certificate {CertificateId}",
                             metric.CertificateId);
            throw;
        }
    }
